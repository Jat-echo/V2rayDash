...
installXray() {
    ...
    if [[ ! -f "/etc/v2ray-agent/xray/xray" ]]; then
        ...
        # GitHub API 限速或网络异常时 version 为空，使用已知可用版本作为兜底
        if [[ -z "${version}" || "${version}" == "null" ]]; then
            version="v25.5.16"
            echoContent yellow " ---> GitHub API 获取版本失败，使用兜底版本 ${version}"
        fi
...
initRealityKey() {
    ...
    if [[ -z "${realityPrivateKey}" ]]; then
        if [[ "${selectCoreType}" == "2" || "${coreInstallType}" == "2" ]]; then
            ...
            if [[ ! -f "/etc/v2ray-agent/xray/xray" ]]; then
                echoContent red " ---> xray 核心未找到，无法生成 Reality 密钥，请检查 xray 是否下载成功"
                exit 1
            fi
